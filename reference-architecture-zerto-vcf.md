...
The Zerto for disaster recovery for VMware Cloud Foundations (VCF) workloads deploys a manual solution where both the protected and recovery sites are in {{site.data.keyword.vpc_short}}. Zerto licenses can be ordered through VMware Solutions portal. This pattern builds on Zerto best practice guidance. For more information, see [Zerto Best Practices](https://help.zerto.com/category/Best_Practices){: external}.
...
2. Deploy a Zerto Virtual Manager appliance and attach it to the management DPG by using the provisioned IP address. Plan and size your deployment. Obtain a license through the VMware Solutions console.
...
4. Configure Zerto Virtual Manager with the IP address range and use the ZVM workflow to deploy the VRAs.
